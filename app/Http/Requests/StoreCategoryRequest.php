...
class StoreCategoryRequest extends FormRequest
{
    ...
    {
        return [
            'title'     =>  'required|min:2|unique:categories,title,'.($this->id ?? ''),
    ...
    public function messages() {
        return [
            ...
            'parent_id.required'  =>  __('Vui lòng chọn :attribute.'),
            'type.required'       =>  __('Vui lòng chọn :attribute.'),
    ...
    public function attributes() {
        return [
            'title'     =>  __('Tên danh mục'),
            'alias'     =>  __('Đường dẫn thân thiện'),
            'parent_id' =>  __('Danh mục cha'),
            'type'      =>  __('Loại danh mục'),
            'ordering'  =>  __('Vị trị sắp xếp')
        ];
    }
}
