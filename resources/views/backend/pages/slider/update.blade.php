...
@section('content')
    ...
    @php
      ...
      $slider_title_image  = json_decode( $slider->title_image );
      $slider_alt_image    = json_decode( $slider->alt_image );
      $slider_button_title = json_decode( $slider->button_title );
      $slider_button_link  = json_decode( $slider->button_link );
    @endphp

    <!-- Main content -->
    <section class="content">
      <form role="form" action="{{ route('slider.update',$slider->id) }}" method="POST" enctype="multipart/form-data" accept-charset="utf-8">
        @csrf
        @method('PUT')
            ...
            <div class="card card-primary">
              ...
              <div class="card-body">
                <div class="form-group">
                  ...
                  <input type="text" id="title" name="title" value="{{ $slider->title ?? old('title') }}" class="form-control" required placeholder="Nhập tiêu đề" oninvalid="this.setCustomValidity('Vui lòng nhập tiêu đề.')" oninput="setCustomValidity('')">
                  @if( $errors->has('title') )
                ...
                <div class="card card-solid">
                  ...
                  <div class="card-body pb-0">
                            ...
                            <div class="card bg-light">
                              ...
                              <div class="card-body pt-0">
                                ...
                                <div class="form-group">
                                  ...
                                  <input type="text" id="title_image" name="title_image[]" placeholder="Nhập tiêu đề hình ảnh" class="form-control" value="{{ $slider_title_image[$key] ?? '' }}" required oninvalid="this.setCustomValidity('Vui lòng nhập tiêu đề.')" oninput="setCustomValidity('')">
                                </div>

                                <div class="form-group">
                                  <label for="alt_image">Mô Tả Hình Ảnh</label>
                                  <input type="text" id="alt_image" name="alt_image[]" placeholder="Nhập mô tả hình ảnh" class="form-control" value="{{ $slider_alt_image[$key] ?? '' }}" required oninvalid="this.setCustomValidity('Vui lòng nhập mô tả.')" oninput="setCustomValidity('')">
                                </div>

                                <div class="form-group">
                                  <label for="button_title">Button Tiêu Đề</label>
                                  <input type="text" id="button_title" name="button_title[]" placeholder="Nhập button tiêu đề" class="form-control" value="{{ $slider_button_title[$key] ?? '' }}">
                                </div>

                                <div class="form-group">
                                  <label for="button_link">Button Đường Dẫn</label>
                                  <input type="text" id="button_link" name="button_link[]" placeholder="Nhập button đường dẫn" class="form-control" value="{{ $slider_button_link[$key] ?? '' }}">
                                </div>
                              </div>
                    ...
                    <div class="form-group text-right">
                      @if( !empty( $slider_images ) && count( $slider_images ) > 0 )
                        <button class="btn-clone-thumbnail btn btn-info" data-count="{{ count( $slider_images ) }}" type="button"><i class="fas fa-plus"></i> Thêm Hình Ảnh</button>
                      @else
