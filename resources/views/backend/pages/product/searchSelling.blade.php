		...
		@foreach($products as $key => $product)
			...
			<li class="ul-item-product-selling {{$product->id}}">
                ...
                <div  class="icheck-primary d-inline ml-2">
                  	<input type="checkbox" value="{{$product->id}}" name="applies_value[{{$product->id}}]['id']" id="todoCheck-product-selling-{{ $key }}">
                  	<label for="todoCheck-product-selling-{{ $key }}"></label>
                </div>
...
@else
	<div class="modal fade" id="modal-lg-product-selling">
		    ...
		    <div class="modal-content">
		      	...
		      	<div class="modal-body">
		        	<form action="" id="formAddItem" method="post">
						<div class="input-group block-search-appliesto inProductSelling">
		                  	<input type="text" class="form-control">
		                  	<span class="input-group-append">
		                    	<button type="button" class="btn btn-info btn-flat product_search" search="inProductSelling" is-append="1">Tìm Kiếm</button>
		                  	</span>
		                </div>

		                <div class="block-product-selling-list">
							<ul class="todo-list" data-widget="todo-list">
								@foreach($products as $key => $product)
									@php
