@if(isset($isAppend) && $isAppend)
	<ul class="todo-list" data-widget="todo-list">
		@foreach($products as $key => $product)
			...
			<li class="ul-item {{$product->id}}">
                <span class="handle">
                  	<i class="fas fa-ellipsis-v"></i>
                  	<i class="fas fa-ellipsis-v"></i>
                </span>
                <div  class="icheck-primary d-inline ml-2">
                  	<input type="checkbox" value="{{$product->id}}" name="applies_value[{{$product->id}}]['id']" id="todoCheck-product-{{ $key }}">
                  	<label for="todoCheck-product-{{ $key }}"></label>
                </div>
		    ...
		    <div class="modal-content">
		      	...
		      	<div class="modal-body">
		        	<form action="" id="formAddItem" method="post">
						<div class="input-group block-search-appliesto inProduct">
		                  	<input type="text" class="form-control">
		                  	<span class="input-group-append">
		                    	<button type="button" class="btn btn-info btn-flat product_search" search="inProduct" is-append="1">Tìm Kiếm</button>
		                  	</span>
		                </div>
