		...
		@foreach($products as $key => $product)
			...
			<li class="ul-item-product-new {{$product->id}}">
                ...
                <div  class="icheck-primary d-inline ml-2">
                  	<input type="checkbox" value="{{$product->id}}" name="applies_value[{{$product->id}}]['id']" id="todoCheck-product-new-{{ $key }}">
                  	<label for="todoCheck-product-new-{{ $key }}"></label>
                </div>
...
@else
	<div class="modal fade" id="modal-lg-product-new">
		    ...
		    <div class="modal-content">
		      	...
		      	<div class="modal-body">
		        	<form action="" id="formAddItem" method="post">
						<div class="input-group block-search-appliesto inProductNew">
		                  	<input type="text" class="form-control">
		                  	<span class="input-group-append">
		                    	<button type="button" class="btn btn-info btn-flat product_search" search="inProductNew" is-append="1">Tìm Kiếm</button>
		                  	</span>
		                </div>

		                <div class="block-product-new-list">
							<ul class="todo-list" data-widget="todo-list">
								@foreach($products as $key => $product)
									@php
