		...
		@foreach($hots as $key => $hot)
			...
			<li class="ul-item {{$hot->id}}">
                ...
                <div  class="icheck-primary d-inline ml-2">
                  	<input type="checkbox" value="{{$hot->id}}" name="applies_value[{{$hot->id}}]['id']" id="todoCheck-hot-{{ $key }}">
                  	<label for="todoCheck-hot-{{ $key }}"></label>
                </div>
...
@else
	<div class="modal fade" id="modal-lg-hot">
		    ...
		    <div class="modal-content">
		      	...
		      	<div class="modal-body">
		        	<form action="" id="formAddItem" method="post">
						<div class="input-group block-search-appliesto inHot">
		                  	<input type="text" class="form-control">
		                  	<span class="input-group-append">
		                    	<button type="button" class="btn btn-info btn-flat hot_search" search="inHot" is-append="1">Tìm Kiếm</button>
		                  	</span>
		                </div>

		                <div class="block-hot-list">
							<ul class="todo-list" data-widget="todo-list">
								@foreach($hots as $key => $hot)
									@php
										$images = $hot->images ?? asset('assets/admin/dist/img/no_image.png');
